...
export default function JimengGenerationHistoryPage() {
  ...
  const downloadCandidate = async (candidate: JimengVideoCandidate) => {
    ...
    try {
      ...
      a.href = url;
      a.download = candidate.video_filename;
      a.click();
      URL.revokeObjectURL(url);
      setNotice(`已开始下载：${candidate.video_filename}`);
    } catch (caught) {
      ...
      <section className="glass-panel sticky top-0 z-20 rounded-xl bg-app-bg/95 px-5 py-4 backdrop-blur-xl">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-text-secondary">按项目、分镜、状态和锁定状态筛选多个候选，便于对比与定稿。</p>
          </div>
          <div className="rounded-lg border border-glass-border bg-black/20 px-3 py-2 text-xs text-text-muted">
        ...
        {loading ? (
          ...
          <div className="grid gap-4 p-4 xl:grid-cols-2 2xl:grid-cols-3">
            {filteredCandidates.map((candidate) => (
              <article key={candidate.id} className="rounded-lg border border-glass-border bg-black/20 p-4">
                <div className="aspect-video overflow-hidden rounded-md border border-glass-border bg-black/40">
                  {jimengMediaUrl(candidate.video_path) ? (
                    <video src={jimengMediaUrl(candidate.video_path)} controls className="h-full w-full object-contain" />
                  ) : jimengMediaUrl(candidate.thumbnail_path) ? (
                    <div
                      role="img"
                      aria-label={candidate.video_filename}
                      className="h-full w-full bg-cover bg-center"
                      style={{ backgroundImage: `url("${jimengMediaUrl(candidate.thumbnail_path).replace(/"/g, '\\"')}")` }}
                    />
                  ) : (
                ...
                <div className="mt-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="truncate text-sm font-semibold text-foreground">{candidate.video_filename}</h3>
                    <p className="mt-1 text-xs text-text-muted">
                      项目 {projectById.get(candidate.project_id)?.name ?? candidate.project_id} · 分镜 {candidate.shot_id.slice(0, 8)}
                    </p>
                  </div>
                  {candidate.is_locked ? <Lock size={14} className="shrink-0 text-emerald-300" /> : <LockOpen size={14} className="shrink-0 text-text-muted" />}
                </div>
                <p className="mt-2 text-xs leading-5 text-text-secondary">{compact(candidate.source_url ?? candidate.video_path, 100)}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => setDefault(candidate)}
                    className="inline-flex items-center gap-2 rounded-md border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary hover:bg-primary/15"
                  >
                    <ShieldCheck size={13} />
                    设为默认
                  </button>
