...
export default function LlmProviderPanel({
  ...
  const isJiasuApiProvider = activeProvider ? activeProvider.id === "jiasuapi" : false;
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate font-mono text-[11px] text-text-muted">
                  {provider.id === "jiasuapi" ? "佳速 API 官方服务" : provider.base_url || "未设置请求地址"}
                </span>
              </span>
              <span
                className={clsx(
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-glass-border bg-surface-inset p-4">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                ...
                {isJiasuApiProvider ? (
                  <div className="space-y-2 rounded-md border border-glass-border bg-surface-card px-3 py-2 md:col-span-2">
                    <span className="block text-xs font-medium text-text-muted">官方地址</span>
                    <a
                      href={JIASU_OFFICIAL_URL}
                      target="_blank"
                      rel="noreferrer"
                      className="break-all font-mono text-sm text-primary hover:text-primary/80"
                    >
                      https://jiasuapi.com/
                    </a>
                    <p className="text-xs leading-5 text-text-muted">默认佳速 API 的请求地址由程序内置，不在页面显示或修改。</p>
                  </div>
                ) : (
