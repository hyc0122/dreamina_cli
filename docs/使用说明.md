...
即梦 CLI 需要排队时，系统会按返回状态轮询。失败任务会标红，并自动跳到下一个任务，不会卡住整个队列。
...
全局画风在“资产管理”的生图设置里，只用于角色、场景、道具生图。它不会参与分镜视频生成，所以不和全局视频生成模板冲突。它是有关联的：单个资产生图和批量资产生图都会把“对应类型前缀 + 全局画风”作为 `extra_prompt` 发给后端，后端再和资产详情描述、资产参数配置组成最终 text2image 提示词。
...
```text
对应类型前缀 + 全局画风 + 单人/群演/场景画风 + 资产详情描述/生图提示词 + 资产参数配置
```
...
资产管理有独立的默认资产画幅，默认 `16:9`，只影响新建资产和批量文件上传创建出来的资产；它和剧本的默认视频画幅是两套设置，互不影响。分辨率在资产详情面板右侧，与生图模型、画幅放在同一行设置；生图设置弹窗里编辑默认资产画幅、画风风格和三类前缀，不再放分辨率，避免同一个参数出现在两个位置。生图设置里也可以选择风格库中的风格，系统会把该风格的“风格提示词”附加到指定画风编辑框中；这个动作只读取风格库内容，不会修改风格库本身。
...
“即梦设置”页面有账号池。每个账号会使用独立的 CLI profile 目录保存登录状态，互不覆盖。

源码运行时，账号 profile 默认保存在：

```text
G:\漫剧\LumenX\dreamina_cli\backend\data\output\jimeng\cli_profiles\<account_id>
```

打包 EXE 后，账号 profile 默认保存在：

```text
EXE同级目录\data\output\jimeng\cli_profiles\<account_id>
```

设置页顶部会显示所有账号积分总额；每张账号卡片会显示该账号的积分、用户名、用户 ID、VIP、到期时间和最近错误。单独提交和批量提交时，可以选择使用哪个账号进入队列。
...
资产管理的“生图设置”里有一个全局画风，以及三套可切换编辑的前缀：

- 人物提示词前缀
- 场景提示词前缀
- 道具提示词前缀

全局画风和下方专属前缀都会发给大模型。三个前缀按钮在同一排，点击后共用下方的大编辑框。资产生图时会按资产类型自动选择对应前缀。实际发送给即梦图片模型的顺序是：

```text
对应类型图片指令前缀 + 全局画风 + 单人/群演/场景画风 + 资产详情描述/生图提示词 + 资产参数配置
```
...
- 多账号 CLI profile：`runtime_data\output\jimeng\cli_profiles`
...
```text
对应类型前缀提示词 + 全局画风 + 资产详情描述/生图提示词 + 资产参数配置
```

- 对应类型前缀提示词分为人物、场景、道具三类，分别用于约束不同资产的生成重点。
- 生图设置中的资产风格可以附加到“全局画风”，这是读取资产风格库内容，不会修改剧本风格库。
...
runtime_data\output\jimeng\cli_profiles\<account_id>
```
...
升级程序时不要删除 `data` 目录，否则项目、缓存、生成视频、上传资产和即梦账号 profile 都会丢失。
