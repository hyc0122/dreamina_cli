...
**预计CLI将转向长期支持，如有后续变更计划会在本文档，即梦首页CLI入口处进行同步通知**

*以下所有安装指令都推荐让你的AGENT帮忙处理，**当然你也可以让 AGENT 直接阅读这个文档～ *

评论排查问题前请先给出执行的命令，报错的描述，位于\~/\.dreamina\_cli/logs/下的日志，否则无法排查

优先更新CLI，重试相关命令，你遇到的问题很有可能已经在新版本解决

业务相关问题，功能提需请找@张成

更新日志（Update）

**更新方式：curl \-fsSL ****https://jimeng\.jianying\.com/cli**** \| bash**
...
- 新增：支持seedance2\.0fast\_vip以及seedance2\.0\_vip通道提速，畅快生成
...
1. **执行登录命令：**运行以下命令以发起登录流程：
    ...
    该命令会自动拉起你的默认浏览器，引导你完成登录授权。**如果你的机器上没有浏览器，dreamina\_cli 会引导你如何手动导入登录态，跟着指引 step by step 完成。**
