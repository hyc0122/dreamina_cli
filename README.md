...
- 即梦设置：CLI 路径、一键安装 CLI、账号池、登录/退出、积分检查、能力检测、全局视频生成模板说明。
- 多账号：每个即梦账号使用独立 CLI profile，设置页显示单账号积分、用户名、VIP 信息和所有账号积分总额。
- 提交参数：视频模型、时长、画幅、分辨率、轮询秒数都在提交位置设置；单独提交在右侧视频预览上方，批量提交在批量参数弹窗中设置。
- 提交账号：单独提交和批量提交都可以选择使用哪个即梦账号。
...
3. 在“即梦设置”的账号池中新增账号，点击登录并完成即梦授权，再查询积分。
...
10. 单独提交可在右侧预览面板设置本次参数和提交账号后提交当前分镜；批量提交先勾选分镜，再打开“批量参数”确认本次生成参数并加入队列。
11. 进入“即梦排队”，手动开始队列。
...
设置页顶部会显示所有账号积分总额；每张账号卡片会显示该账号的积分、用户名、VIP 和到期时间。单独提交和批量提交时，可以选择使用哪个账号进入队列。
